...
Sequence::Sequence(const Sequence& other) {
	m_size = other.m_size;
	m_max = other.m_max;
	m_newseq = new ItemType[m_size];
	for (int j = 0; j < m_size; j++) {
		m_newseq[j] = other.m_newseq[j];
	}
}

//Assignment Operator
Sequence& Sequence::operator=(const Sequence& nhs) {
	if (this != &nhs) {
		delete[] m_newseq;
		m_max = nhs.m_max;
		m_size = nhs.m_max;
...
//find the bigger size
	...
	for (int i = 0; i < biggest; i++) {
		var = *(m_newseq + i);
		*(m_newseq + i) = *(other.m_newseq + i);
		*(other.m_newseq + i) = var;
	}
	//swap the sizes
